...
__device__ float whrtfs[131][360][560];
...
extern "C" float* resp_imp(int elev, int azim,char* filtros[], int numFiltros, float** G, int* G_size, int* resultLength);
 ...
 *	esparsos.
 */
float* resp_imp(int elev, int azim, char* filtros[], int numFiltros, float** G, int* G_size, int* resultLength) {
	...
	//copyToWhrtf<<<1, maxRSize-maxFilterBankLength+1>>>(d_resultado, maxFilterBankLength, maxRSize, elev, azim);
