...
void getAllSparseCoefficients(int ear, float** allSparseCoeficients[121][360], int *allSparseCoeficientsSize[121][360]);
...
void getAllSparseCoefficients(int ear, float** allSparseCoeficients[121][360], int *allSparseCoeficientsSize[121][360]) {
	float** Gp = NULL;
	int* Gp_size = (int*) calloc((NUM_FILTROS+1), sizeof(int));
	...
	for (elev = -40; elev <= 80; elev += 10) {
		int minAzimBound = 0;
		int maxAzimBound = 0;
		for (azim = 0; azim < 360; azim++) {
			if (maxAzimBound <= azim) {
				getAzimuthBoundaries(elev, azim, &minAzimBound, &maxAzimBound);
				
				if (minAzimBound != maxAzimBound) {
					free(Ga); Ga = NULL;
					free(Gb); Gb = NULL;
					
					Ga = getCoefSpars(elev, minAzimBound, ear, Ga_size);
					Gb = getCoefSpars(elev, maxAzimBound, ear, Gb_size);
					
					allSparseCoeficients[elev+40][azim] = getGp(Ga, Ga_size, Gb, Gb_size, minAzimBound, maxAzimBound, azim, Gp_size);
				} else if (minAzimBound == maxAzimBound) {
					allSparseCoeficients[elev+40][azim] = getCoefSpars(elev, minAzimBound, ear, Gp_size);
				}
			} else if (azim < maxAzimBound) {
				allSparseCoeficients[elev+40][azim] = getGp(Ga, Ga_size, Gb, Gb_size, minAzimBound, maxAzimBound, azim, Gp_size);
			}
			
			allSparseCoeficientsSize[elev+40][azim] = (int*) calloc((NUM_FILTROS+1), sizeof(int));
			for (int i = 0; i < 5; i++) {
				allSparseCoeficientsSize[elev+40][azim][i] = Gp_size[i];
			}
		}
	}
	...
	for (elev = -40; elev <= 80; elev++) {
		...
		if (!knownElev) {
			for (azim = 0; azim < 360; azim++) {
				...
				allSparseCoeficients[elev+40][azim] = getGp(allSparseCoeficients[minElev+40][azim], allSparseCoeficientsSize[minElev+40][azim], allSparseCoeficients[maxElev+40][azim], allSparseCoeficientsSize[maxElev+40][azim], minElev, maxElev, elev, Gp_size);
